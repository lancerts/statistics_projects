function [x,f,exitflag,output] = minFunc_time(funObj,x0,options,varargin)
...
if ~isempty(outputFcn)
    duration_time=toc;
    stop = outputFcn(x,'init',0,funEvals,f,[],[],g,[],max(abs(g)),duration_time,varargin{:});
    if stop
        exitflag=-1;
...
for i = 1:maxIter
    ...
    % Report the beta_estimation at
    if trace.termination_time==0
        if  toc>options.MAX_TIME
            trace.termination_time=toc;
            trace.termination_beta=x;
            trace.termination_iter=i;
        end
    end
    % Output Function
    if ~isempty(outputFcn)
        stop = outputFcn(x,'iter',i,funEvals,f,t,gtd,g,d,optCond,duration_time,varargin{:});
        if stop
            exitflag=-1;
            msg = 'Stopped by output function';
            break;
...
% If the algorithm converged or stopped before termination_time
if  toc<=options.MAX_TIME
    trace.termination_time=toc;
    trace.termination_beta=x;
    trace.termination_iter=i;
...
% Output Function
if ~isempty(outputFcn)
    outputFcn(x,'done',i,funEvals,f,t,gtd,g,d,max(abs(g)),duration_time,varargin{:});
end
