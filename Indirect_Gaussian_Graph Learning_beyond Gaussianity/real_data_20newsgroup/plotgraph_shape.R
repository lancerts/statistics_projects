...
if (dim(W_est)[1]==3*k){
sub_net %v% "Type" <-c (rep("Continuous",k),rep("Count",k),rep("Binary",k))[ -inds]
sub_net %v% "Color" <-c (rep("phono",k),rep("steelblue",k),rep("grey",k))[ -inds]

ggnet2(sub_net,label=TRUE, node.size=degree(sub_net), 
       label.size=1.5, label.color=labelcolor, max_size = max(degree(sub_net))/5,color = "Type",
       shape="Type", alpha="phono", alpha.palette=0.7,  palette = "Set2", 
       edge.color=edgecolor, edge.size=0.2,  legend.size = 6) + guides(size=FALSE)
}   
